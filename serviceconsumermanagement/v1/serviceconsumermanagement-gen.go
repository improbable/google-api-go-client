...
// configured tenant project to a tenancy unit.
...
// the tenancy unit as a new tenant resource.
type AttachTenantProjectRequest struct {
	...
	// tenancy units, this is the tag of a tenant resource under the tenancy
	// unit for the managed service's service producer project. The reserved
...
type CreateTenancyUnitRequest struct {
	// TenancyUnitId: Optional. Optional service producer-provided
	// identifier of the tenancy unit. Must be no longer than 40 characters
	// and preferably URI friendly. If it isn't provided, a UID for the
	// tenancy unit is automatically generated. The identifier must be
	// unique across a managed service. If the tenancy unit already exists
	// for the managed service and service consumer pair, calling
...
type PolicyBinding struct {
	// Members: Uses the same format as in IAM policy. `member` must include
	// both a prefix and ID. For example, `user:{emailId}`,
	// `serviceAccount:{emailId}`, `group:{emailId}`.
	Members []string `json:"members,omitempty"`

	// Role: Role. (https://cloud.google.com/iam/docs/understanding-roles)
...
type ServiceAccountConfig struct {
	...
	// project. The email format of the service account is
	// "@.iam.gserviceaccount.com". This account ID must be unique within
	// tenant project and service producers have to guarantee it. The ID
	// must be 6-30 characters long, and match the following regular
...
type TenancyUnit struct {
	...
	// owning this tenancy unit. For example
	// 'serviceconsumermanagement.googleapis.com'.
	Service string `json:"service,omitempty"`

	// TenantResources: Resources constituting the tenancy unit. There can
	// be at most 512 tenant resources in a tenancy unit.
...
// added to the specified tenancy unit and its initial configuration and
// properties. A project lien is created for the tenant project to
// prevent the tenant project from being deleted accidentally. The lien
// is deleted as part of tenant project removal.
type TenantProjectConfig struct {
	...
	// Folder: Folder where project in this tenancy unit must be located
	...
	// this project during provisioning. If any of these services can't be
	// activated, the request fails. For example:
...
type TenantProjectPolicy struct {
	...
	// `roles/owner`. Among the list of members for `roles/owner`, at least
	// one of them must be either the `user` or `group` type.
...
// of results returned by this request. Currently, the default maximum
// is set to 1000. If `page_size` isn't provided or the size provided is
// a number larger than 1000, it's automatically set to 1000.
...
// for querying tenancy units. Your `{expression}` must be in the
// format: `field_name=literal_string`. The `field_name` is the name of
// the field you want to compare. Supported fields are
...
// included in the result set. For example, `tenant_resources.tag=xyz
// AND tenant_resources.resource=projects/123456`
...
// project. This project must exist in an active state and have the
// original owner account. The caller must have permission to add a
// project to the given tenancy unit. The configuration is applied, but
// any existing settings on the project aren't modified. Specified
// policy bindings are applied. Existing bindings aren't modified.
// Specified services are activated. No service is deactivated. If
// specified, new billing configuration is applied. Omit a billing
// configuration to keep the existing one. A service account in the
// project is created if previously non existed. Specified labels will
// be appended to tenant project, note that the value of existing label
// key will be updated if the same label key is requested. The specified
// folder is ignored, as moving a tenant project to a different folder
...
func (c *ServicesTenancyUnitsApplyProjectConfigCall) doRequest(alt string) (*http.Response, error) {
	...
	return gensupport.SendRequest(googleapi.MethodIDToContext(c.ctx_, "serviceconsumermanagement.services.tenancyUnits.applyProjectConfig"), c.s.client, req)
}
...
// new tenant resource. The project could either be the tenant project
// reserved by calling `AddTenantProject` under a tenancy unit of a
// service producer's project of a managed service, or from a separate
// project. The caller is checked against a set of permissions as if
// calling `AddTenantProject` on the same service consumer. To trigger
// the attachment, the targeted tenant project must be in a folder. Make
...
func (c *ServicesTenancyUnitsAttachProjectCall) doRequest(alt string) (*http.Response, error) {
	...
	return gensupport.SendRequest(googleapi.MethodIDToContext(c.ctx_, "serviceconsumermanagement.services.tenancyUnits.attachProject"), c.s.client, req)
}
...
// Create: Creates a tenancy unit with no tenant resources. If tenancy
// unit already exists, it will be returned, however, in this case,
...
// tenant resource tag. The mothod removes a project lien with a
// 'TenantManager' origin if that was added. It will then attempt to
// delete the project. If that operation fails, this method also fails.
// After the project has been deleted, the tenant resource state is set
// to DELETED. To permanently remove resource metadata, call the
...
func (c *ServicesTenancyUnitsDeleteProjectCall) doRequest(alt string) (*http.Response, error) {
	...
	return gensupport.SendRequest(googleapi.MethodIDToContext(c.ctx_, "serviceconsumermanagement.services.tenancyUnits.deleteProject"), c.s.client, req)
}
...
// consumer. This method shouldn't be used in a service producer's
...
// tenant resource tag. The method removes the project lien with
// 'TenantManager' origin if that was added. It then attempts to delete
// the project. If that operation fails, this method also fails. Calls
// to remove already removed or non-existent tenant project succeed.
...
// project. The project must be in a DELETED state. There are no
// guarantees that an undeleted project will be in a fully restored and
...
func (c *ServicesTenancyUnitsUndeleteProjectCall) doRequest(alt string) (*http.Response, error) {
	...
	return gensupport.SendRequest(googleapi.MethodIDToContext(c.ctx_, "serviceconsumermanagement.services.tenancyUnits.undeleteProject"), c.s.client, req)
}
