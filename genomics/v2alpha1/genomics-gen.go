...
type CheckInResponse struct {
	...
	// The backend will allow for network transmission time and other
	// delays, but the worker must attempt to transmit the extension request
...
// for the Cloud Genomics service account or the request will fail.
...
// the worker and the time of occurance.
...
// contain a google.genomics.v2alpha1.Metadata object describing the
// status of the pipeline execution. The [response] field will contain a
...
// automatically when the Cloud Genomics API is first enabled, but if
...
func (c *ProjectsWorkersCheckInCall) doRequest(alt string) (*http.Response, error) {
	...
	return gensupport.SendRequest(googleapi.MethodIDToContext(c.ctx_, "genomics.projects.workers.checkIn"), c.s.client, req)
}
