...
const mtlsBasePath = "https://slides.mtls.googleapis.com/"
...
// last row of [ 0 0 1 ] to transform source coordinates (x,y) into
// destination coordinates (x', y') according to: x' x = shear_y scale_y
// translate_y 1 [ 1 ] After transformation, x' = scale_x * x + shear_x
// * y + translate_x; y' = scale_y * y + shear_y * x + translate_y; This
...
type AffineTransform struct {
	...
	// centimeter and thus there are 914,400 EMUs per inch, and 12,700 EMUs
	// per point.
...
type ColorStop struct {
	...
	// band measured in percentage. The value should be in the interval
	// [0.0, 1.0].
...
type CreateLineRequest struct {
	// Category: The category of the line to be created. The exact line type
	// created is determined based on the category and how it's routed to
	// connect to other page elements. If you specify both a `category` and
	// a `line_category`, the `category` takes precedence. If you do not
	// specify a value for `category`, but specify a value for
	// `line_category`, then the specified `line_category` value is used. If
	// you do not specify either, then STRAIGHT is used.
	//
	// Possible values:
	...
	// LineCategory: The category of the line to be created. *Deprecated*:
	// use `category` instead. The exact line type created is determined
	// based on the category and how it's routed to connect to other page
	// elements. If you specify both a `category` and a `line_category`, the
	// `category` takes precedence.
	//
	// Possible values:
	//   "STRAIGHT" - Straight connectors, including straight connector 1.
	// The is the default category when one is not specified.
	//   "BENT" - Bent connectors, including bent connector 2 to 5.
	//   "CURVED" - Curved connectors, including curved connector 2 to 5.
...
// paragraphs that overlap with the given text index range. The nesting
// level of each paragraph will be determined by counting leading tabs
// in front of each paragraph. To avoid excess space between the bullet
// and the corresponding paragraph, these leading tabs are removed by
// this request. This may change the indices of parts of the text. If
// the paragraph immediately before paragraphs being updated is in a
// list with a matching preset, the paragraphs being updated are added
...
type CreateParagraphBulletsRequest struct {
	...
	// `ALPHA` and `ROMAN` numeric glyphs for the first 3 list nesting
	// levels, followed by parenthesis.
...
type CreateShapeRequest struct {
	...
	// regex `[a-zA-Z0-9_]`); remaining characters may include those as well
	// as a hyphen or colon (matches regex `[a-zA-Z0-9_-:]`). The length of
	// the ID must not be less than 5 or greater than 50. If empty, a unique
	...
	// ECMA-376 ST_ShapeType 'downArrowCallout'
	...
	// ECMA-376 ST_ShapeType 'horizontalScroll'
	//   "IRREGULAR_SEAL_1" - Irregular seal 1 shape. Corresponds to
	// ECMA-376 ST_ShapeType 'irregularSeal1'
	//   "IRREGULAR_SEAL_2" - Irregular seal 2 shape. Corresponds to
	// ECMA-376 ST_ShapeType 'irregularSeal2'
	...
	// ECMA-376 ST_ShapeType 'leftArrowCallout'
	...
	// ECMA-376 ST_ShapeType 'leftRightArrow'
	//   "LEFT_RIGHT_ARROW_CALLOUT" - Callout left right arrow shape.
	// Corresponds to ECMA-376 ST_ShapeType 'leftRightArrowCallout'
	...
	// ECMA-376 ST_ShapeType 'quadArrowCallout'
	...
	// to ECMA-376 ST_ShapeType 'round1Rect'
	...
	// shape. Corresponds to ECMA-376 ST_ShapeType 'round2SameRect'
	...
	// to ECMA-376 ST_ShapeType 'snip1Rect'
	//   "SNIP_2_DIAGONAL_RECTANGLE" - Two diagonal snip corner rectangle
	// shape. Corresponds to ECMA-376 ST_ShapeType 'snip2DiagRect'
	//   "SNIP_2_SAME_RECTANGLE" - Two same-side snip corner rectangle
	// shape. Corresponds to ECMA-376 ST_ShapeType 'snip2SameRect'
	...
	// ECMA-376 ST_ShapeType 'upArrowCallout'
	...
	// to ECMA-376 ST_ShapeType 'wedgeEllipseCallout'
	//   "WEDGE_RECTANGLE_CALLOUT" - Callout wedge rectangle shape.
	// Corresponds to ECMA-376 ST_ShapeType 'wedgeRectCallout'
	//   "WEDGE_ROUND_RECTANGLE_CALLOUT" - Callout wedge round rectangle
	// shape. Corresponds to ECMA-376 ST_ShapeType 'wedgeRoundRectCallout'
	...
	// ECMA-376 ST_ShapeType 'flowChartConnector'
	//   "FLOW_CHART_DECISION" - Decision flow shape. Corresponds to
	// ECMA-376 ST_ShapeType 'flowChartDecision'
	...
	// ECMA-376 ST_ShapeType 'flowChartDocument'
	...
	// Corresponds to ECMA-376 ST_ShapeType 'flowChartInternalStorage'
	//   "FLOW_CHART_MAGNETIC_DISK" - Magnetic disk flow shape. Corresponds
	// to ECMA-376 ST_ShapeType 'flowChartMagneticDisk'
	//   "FLOW_CHART_MAGNETIC_DRUM" - Magnetic drum flow shape. Corresponds
	// to ECMA-376 ST_ShapeType 'flowChartMagneticDrum'
	//   "FLOW_CHART_MAGNETIC_TAPE" - Magnetic tape flow shape. Corresponds
	// to ECMA-376 ST_ShapeType 'flowChartMagneticTape'
	...
	// Corresponds to ECMA-376 ST_ShapeType 'flowChartManualOperation'
	...
	// to ECMA-376 ST_ShapeType 'flowChartMultidocument'
	...
	// Corresponds to ECMA-376 ST_ShapeType 'flowChartOnlineStorage'
	...
	// Corresponds to ECMA-376 ST_ShapeType 'flowChartSummingJunction'
	//   "FLOW_CHART_TERMINATOR" - Terminator flow shape. Corresponds to
	// ECMA-376 ST_ShapeType 'flowChartTerminator'
	...
	// ECMA-376 ST_ShapeType 'ellipseRibbon2'
...
// spreadsheets.readonly, spreadsheets, drive.readonly, drive.file, or
// drive OAuth scopes.
type CreateSheetsChartRequest struct {
	...
	// ElementProperties: The element properties for the chart. When the
	// aspect ratio of the provided size does not match the chart aspect
	...
	// spreadsheet. When not specified, the chart will be an image that is
	// not linked.
	//
	// Possible values:
	...
	// ObjectId: A user-supplied object ID. If specified, the ID must be
	// unique among all pages and page elements in the presentation. The ID
	// should start with a word character [a-zA-Z0-9_] and then followed by
	// any number of the following characters [a-zA-Z0-9_-:]. The length of
	// the ID should not be less than 5 or greater than 50. If empty, a
...
type CreateSlideRequest struct {
	...
	// based on the *current master*, which is one of the following: - The
	// master of the previous slide index. - The master of the first slide,
...
type CreateTableRequest struct {
	...
	// ElementProperties: The element properties for the table. The table
	// will be created at the provided size, subject to a minimum size. If
	// no size is provided, the table will be automatically sized. Table
	// transforms must have a scale of 1 and no shear components. If no
...
// CreateVideoRequest: Creates a video. NOTE: Creating a video from
// Google Drive requires that the requesting app have at least one of
// the drive, drive.readonly, or drive.file OAuth scopes.
type CreateVideoRequest struct {
	// ElementProperties: The element properties for the video. The
	// PageElementProperties.size property is optional. If you don't specify
	// a size, a default size is chosen by the server. The
	// PageElementProperties.transform property is optional. The transform
	// must not have shear components. If you don't specify a transform, the
	// video will be placed at the top left corner of the page.
	ElementProperties *PageElementProperties `json:"elementProperties,omitempty"`

	// Id: The video source's unique identifier for this video. e.g. For
...
// container. For example, an Image. The crop properties is represented
// by the offsets of four edges which define a crop rectangle. The
// offsets are measured in percentage from the corresponding edges of
// the object's original bounding rectangle towards inside, relative to
// the object's original dimensions. - If the offset is in the interval
// (0, 1), the corresponding edge of crop rectangle is positioned inside
// of the object's original bounding rectangle. - If the offset is
// negative or greater than 1, the corresponding edge of crop rectangle
// is positioned outside of the object's original bounding rectangle. -
// If the left edge of the crop rectangle is on the right side of its
// right edge, the object will be flipped horizontally. - If the top
// edge of the crop rectangle is below its bottom edge, the object will
// be flipped vertically. - If all offsets and rotation angle is 0, the
// object is not cropped. After cropping, the content in the crop
...
type CropProperties struct {
	...
	// radians. Rotation angle is applied after the offset.
	Angle float64 `json:"angle,omitempty"`

	// BottomOffset: The offset specifies the bottom edge of the crop
	// rectangle that is located above the original bounding rectangle
	// bottom edge, relative to the object's original height.
	BottomOffset float64 `json:"bottomOffset,omitempty"`

	// LeftOffset: The offset specifies the left edge of the crop rectangle
	// that is located to the right of the original bounding rectangle left
	// edge, relative to the object's original width.
	LeftOffset float64 `json:"leftOffset,omitempty"`

	// RightOffset: The offset specifies the right edge of the crop
	// rectangle that is located to the left of the original bounding
	// rectangle right edge, relative to the object's original width.
	RightOffset float64 `json:"rightOffset,omitempty"`

	// TopOffset: The offset specifies the top edge of the crop rectangle
	// that is located below the original bounding rectangle top edge,
...
// DeleteObjectRequest: Deletes an object, either pages or page
...
// paragraphs that overlap with the given text index range. The nesting
// level of each paragraph will be visually preserved by adding indent
// to the start of the corresponding paragraph.
...
type DeleteTableColumnRequest struct {
	...
	// will be deleted. The column this cell spans will be deleted. If this
	// is a merged cell, multiple columns will be deleted. If no columns
...
type DeleteTableRowRequest struct {
	...
	// be deleted. The row this cell spans will be deleted. If this is a
	// merged cell, multiple rows will be deleted. If no rows remain in the
...
type DeleteTextRequest struct {
	...
	// deleted from a table cell. If present, the object_id must refer to a
	// table.
	CellLocation *TableCellLocation `json:"cellLocation,omitempty"`
	...
	// TextRange: The range of text to delete, based on TextElement indexes.
	// There is always an implicit newline character at the end of a shape's
	// or table cell's text that cannot be deleted. `Range.Type.ALL` will
	// use the correct bounds, but care must be taken when specifying
	// explicit bounds for range types `FROM_START_INDEX` and `FIXED_RANGE`.
	// For example, if the text is "ABC", followed by an implicit newline,
	// then the maximum value is 2 for `text_range.start_index` and 3 for
...
// DuplicateObjectRequest: Duplicates a slide or page element. When
// duplicating a slide, the duplicate slide will be created immediately
...
type DuplicateObjectRequest struct {
	...
	// defines how the IDs of duplicated objects are generated: the keys are
	// the IDs of the original objects and its values are the IDs that will
	// be assigned to the corresponding duplicate object. The ID of the
	// source object's duplicate may be specified in this map as well, using
	// the same value of the `object_id` field as a key and the newly
	// desired ID as the value. All keys must correspond to existing IDs in
	// the presentation. All values must be unique in the presentation and
	// must start with an alphanumeric character or an underscore (matches
	// regex `[a-zA-Z0-9_]`); remaining characters may include those as well
	// as a hyphen or colon (matches regex `[a-zA-Z0-9_-:]`). The length of
	// the new ID must not be less than 5 or greater than 50. If any IDs of
...
// Group: A PageElement kind representing a joined collection of
// PageElements.
type Group struct {
...
type GroupObjectsRequest struct {
	...
	// elements in the presentation. The ID must start with an alphanumeric
	// character or an underscore (matches regex `[a-zA-Z0-9_]`); remaining
	// characters may include those as well as a hyphen or colon (matches
	// regex `[a-zA-Z0-9_-:]`). The length of the ID must not be less than 5
	// or greater than 50. If you don't specify an ID, a unique one is
	// generated.
...
type Image struct {
	...
	// URL effectively accesses the image as the original requester. Access
	// to the image may be lost if the presentation's sharing settings
	// change.
...
type ImageProperties struct {
	...
	// in the interval [-1.0, 1.0], where 0 means no effect. This property
	// is read-only.
	Brightness float64 `json:"brightness,omitempty"`

	// Contrast: The contrast effect of the image. The value should be in
	// the interval [-1.0, 1.0], where 0 means no effect. This property is
	// read-only.
	Contrast float64 `json:"contrast,omitempty"`

	// CropProperties: The crop properties of the image. If not set, the
	// image is not cropped. This property is read-only.
	...
	// not recolored. This property is read-only.
	...
	// completely transparent. This property is read-only.
...
// InsertTableColumnsRequest: Inserts columns into a table. Other
// columns in the table will be resized to fit the new column.
type InsertTableColumnsRequest struct {
	// CellLocation: The reference table cell location from which columns
	// will be inserted. A new column will be inserted to the left (or
	// right) of the column where the reference cell is. If the reference
	// cell is a merged cell, a new column will be inserted to the left (or
	// right) of the merged cell.
	CellLocation *TableCellLocation `json:"cellLocation,omitempty"`
...
type InsertTableRowsRequest struct {
	...
	// be inserted. A new row will be inserted above (or below) the row
...
type InsertTextRequest struct {
	// CellLocation: The optional table cell location if the text is to be
	// inserted into a table cell. If present, the object_id must refer to a
	// table.
	CellLocation *TableCellLocation `json:"cellLocation,omitempty"`

	// InsertionIndex: The index where the text will be inserted, in Unicode
	// code units, based on TextElement indexes. The index is zero-based and
	// is computed from the start of the string. The index may be adjusted
	// to prevent insertions inside Unicode grapheme clusters. In these
	// cases, the text will be inserted immediately after the grapheme
	// cluster.
	InsertionIndex int64 `json:"insertionIndex,omitempty"`
	...
	// current insertion index, including lists and bullets. Text styles for
	// inserted text will be determined automatically, generally preserving
	// the styling of neighboring text. In most cases, the text will be
	// added to the TextRun that exists at the insertion index. Some control
...
// placeholder that will be created on a slide from a specified layout.
type LayoutPlaceholderIdMapping struct {
	// LayoutPlaceholder: The placeholder on a layout that will be applied
	// to a slide. Only type and index are needed. For example, a predefined
	...
	// layout that will be applied to a slide.
	...
	// above that to be created onto a slide. If you specify an ID, it must
	// be unique among all pages and page elements in the presentation. The
	// ID must start with an alphanumeric character or an underscore
	// (matches regex `[a-zA-Z0-9_]`); remaining characters may include
	// those as well as a hyphen or colon (matches regex `[a-zA-Z0-9_-:]`).
	// The length of the ID must not be less than 5 or greater than 50. If
...
// LayoutProperties: The properties of Page are only relevant for pages
...
// LayoutReference: Slide layout reference. This may reference either: -
...
type Line struct {
	// LineCategory: The category of the line. It matches the `category`
...
type LineConnection struct {
	// ConnectedObjectId: The object ID of the connected page element. Some
	// page elements, such as groups, tables, and lines do not have
	// connection sites and therefore cannot be connected to a connector
	// line.
	ConnectedObjectId string `json:"connectedObjectId,omitempty"`

	// ConnectionSiteIndex: The index of the connection site on the
	// connected page element. In most cases, it corresponds to the
	// predefined connection site index from the ECMA-376 standard. More
	// information on those connection sites can be found in the description
	// of the "cnx" attribute in section 20.1.9.9 and Annex H. "Predefined
...
// LineProperties: The properties of the Line. When unset, these fields
// default to values that match the appearance of new lines created in
// the Slides editor.
type LineProperties struct {
	...
	// is no connection. Only lines with a Type indicating it is a
	...
	// defaults for new lines created in the Slides editor.
	...
	// unset, there is no connection. Only lines with a Type indicating it
	// is a "connector" can have a `start_connection`.
...
type Link struct {
	...
	// zero-based index in the presentation. There may not be a slide at
	// this index.
...
// paragraphs associated with a list. A paragraph that is part of a list
// has an implicit reference to that list's ID.
type List struct {
	// ListId: The ID of the list.
	ListId string `json:"listId,omitempty"`

	// NestingLevel: A map of nesting levels to the properties of bullets at
	// the associated level. A list has at most nine levels of nesting, so
	// the possible values for the keys of this map are 0 through 8,
	// inclusive.
...
// MasterProperties: The properties of Page that are only relevant for
// pages with page_type MASTER.
...
type MergeTableCellsRequest struct {
	...
	// stored in the upper-left ("head") cell of the range. If the range is
...
// NotesProperties: The properties of Page that are only relevant for
// pages with page_type NOTES.
type NotesProperties struct {
	// SpeakerNotesObjectId: The object ID of the shape on this notes page
	// that contains the speaker notes for the corresponding slide. The
	// actual shape may not always exist on the notes page. Inserting text
	// using this object ID will automatically create the shape. In this
	// case, the actual shape may have different object ID. The
	// `GetPresentation` or `GetPage` action will always return the latest
	// object ID.
...
type OptionalColor struct {
	...
	// this represents a transparent color.
...
// Outline: The outline of a PageElement. If these fields are unset,
// they may be inherited from a parent placeholder if it exists. If
// there is no parent, the fields will default to the value used for new
// page elements created in the Slides editor, which may depend on the
// page element kind.
type Outline struct {
	...
	// PropertyState: The outline property state. Updating the outline on a
	// page element will implicitly update this field to `RENDERED`, unless
	// another value is specified in the same request. To have no outline on
	// a page element, set this field to `NOT_RENDERED`. In this case, any
	// other outline fields set in the same request will be ignored.
	//
	// Possible values:
	//   "RENDERED" - If a property's state is RENDERED, then the element
	// has the corresponding property when rendered on a page. If the
	// element is a placeholder shape as determined by the placeholder
	// field, and it inherits from a placeholder shape, the corresponding
	// field may be unset, meaning that the property value is inherited from
	// a parent placeholder. If the element does not inherit, then the field
	// will contain the rendered value. This is the default value.
	//   "NOT_RENDERED" - If a property's state is NOT_RENDERED, then the
	// element does not have the corresponding property when rendered on a
	// page. However, the field may still be set so it can be inherited by
	// child shapes. To remove a property from a rendered element, set its
	// property_state to NOT_RENDERED.
	//   "INHERIT" - If a property's state is INHERIT, then the property
	// state uses the value of corresponding `property_state` field on the
	// parent shape. Elements that do not inherit will never have an INHERIT
	// property state.
...
type Page struct {
	...
	// ObjectId: The object ID for this page. Object IDs used by Page and
...
type PageBackgroundFill struct {
	// PropertyState: The background fill property state. Updating the fill
	// on a page will implicitly update this field to `RENDERED`, unless
	// another value is specified in the same request. To have no fill on a
	// page, set this field to `NOT_RENDERED`. In this case, any other fill
...
type PageElement struct {
	...
	// to display alt text. The field is not supported for Group elements.
	...
	// charts are represented as images.
	...
	// display alt text. The field is not supported for Group elements.
	Title string `json:"title,omitempty"`

	// Transform: The transform of the page element. The visual appearance
	// of the page element is determined by its absolute transform. To
	// compute the absolute transform, preconcatenate a page element's
	// transform with the transforms of all of its parent groups. If the
	// page element is not in a group, its absolute transform is the same as
	// the value in this field. The initial transform for the newly created
	// Group is always the identity transform.
...
// PageElementProperties: Common properties for a page element. Note:
// When you initially create a PageElement, the API may modify the
// values of both `size` and `transform`, but the visual size will be
// unchanged.
...
// PageProperties: The properties of the Page. The page will inherit
// properties from the parent page. Depending on the page type the
...
type PageProperties struct {
	...
	// is inherited from a parent page. If the page has no parent, the color
	...
	// background fill is inherited from a parent page if it exists. If the
	// page has no parent, then the background fill defaults to the
...
// ParagraphStyle: Styles that apply to a whole paragraph. If this text
// is contained in a shape with a parent placeholder, then these
// paragraph styles may be inherited from the parent. Which paragraph
// styles are inherited depend on the nesting level of lists: * A
// paragraph not in a list will inherit its paragraph style from the
// paragraph at the 0 nesting level of the list inside the parent
// placeholder. * A paragraph in a list will inherit its paragraph style
// from the paragraph at its corresponding nesting level of the list
// inside the parent placeholder. Inherited paragraph styles are
...
type ParagraphStyle struct {
	...
	// defaults to LEFT_TO_RIGHT since text direction is not inherited.
	//
	// Possible values:
	...
	// line of the paragraph. If unset, the value is inherited from the
	// parent.
	IndentFirstLine *Dimension `json:"indentFirstLine,omitempty"`

	// IndentStart: The amount indentation for the paragraph on the side
	// that corresponds to the start of the text, based on the current text
	// direction. If unset, the value is inherited from the parent.
...
type Placeholder struct {
	...
	// are present in the same page, they would have different index values.
	Index int64 `json:"index,omitempty"`

	// ParentObjectId: The object ID of this shape's parent placeholder. If
	// unset, the parent placeholder shape does not exist, so the shape does
...
type Presentation struct {
	...
	// contains all common page elements and the common properties for a set
	// of layouts. They serve three purposes: - Placeholder shapes on a
	...
	// properties define the common page properties inherited by its
	// layouts. - Any other shapes on the master slide appear on all slides
	// using that master, regardless of their layout.
	Masters []*Page `json:"masters,omitempty"`

	// NotesMaster: The notes master in the presentation. It serves three
	// purposes: - Placeholder shapes on a notes master contain the default
	// text styles and shape properties of all placeholder shapes on notes
	// pages. Specifically, a `SLIDE_IMAGE` placeholder shape contains the
	...
	// notes master appears on all notes pages. The notes master is
	// read-only.
	...
	// Slides: The slides in the presentation. A slide inherits properties
	// from a slide layout.
...
type Range struct {
	...
	// collection. Required for `FIXED_RANGE` and `FROM_START_INDEX` ranges.
	...
	// continues until the end of the collection. The `end_index` must not
	// be specified.
	//   "ALL" - Sets the range to be the whole length of the collection.
	// Both the `start_index` and the `end_index` must not be specified.
	Type string `json:"type,omitempty"`
...
type Recolor struct {
	// Name: The name of the recolor effect. The name is determined from the
	...
	// is a list of color stops. The colors in the gradient will replace the
...
// by replacing it with the latest version of the chart from Google
// Sheets. NOTE: Refreshing charts requires at least one of the
...
// given criteria with the provided image. The images replacing the
// shapes are rectangular after being inserted into the presentation and
// do not take on the forms of the shapes.
type ReplaceAllShapesWithImageRequest struct {
	...
	// ImageReplaceMethod: The image replace method. If you specify both a
	// `replace_method` and an `image_replace_method`, the
	// `image_replace_method` takes precedence. If you do not specify a
	// value for `image_replace_method`, but specify a value for
	...
	// bounds of the original shape and maintains the image's aspect ratio.
	// The rendered size of the image may be smaller than the size of the
	...
	// ImageUrl: The image URL. The image is fetched once at insertion time
	// and a copy is stored for display inside the presentation. Images must
	// be less than 50MB in size, cannot exceed 25 megapixels, and must be
	// in one of PNG, JPEG, or GIF format. The provided URL can be at most 2
	// kB in length. The URL itself is saved with the image, and exposed via
	// the Image.source_url field.
	ImageUrl string `json:"imageUrl,omitempty"`

	// PageObjectIds: If non-empty, limits the matches to page elements only
	// on the given pages. Returns a 400 bad request error if given the page
	// object ID of a notes page or a notes master, or if a page with that
	// object ID doesn't exist in the presentation.
	PageObjectIds []string `json:"pageObjectIds,omitempty"`

	// ReplaceMethod: The replace method. *Deprecated*: use
	// `image_replace_method` instead. If you specify both a
	// `replace_method` and an `image_replace_method`, the
...
// match the given criteria with the provided Google Sheets chart. The
// chart will be scaled and centered to fit within the bounds of the
// original shape. NOTE: Replacing shapes with a chart requires at least
// one of the spreadsheets.readonly, spreadsheets, drive.readonly, or
// drive OAuth scopes.
type ReplaceAllShapesWithSheetsChartRequest struct {
	...
	// replaced. The request will replace all of the shapes that contain the
	// given text.
	ContainsText *SubstringMatchCriteria `json:"containsText,omitempty"`
...
type ReplaceAllTextRequest struct {
	...
	// PageObjectIds: If non-empty, limits the matches to page elements only
	// on the given pages. Returns a 400 bad request error if given the page
	// object ID of a notes master, or if a page with that object ID doesn't
...
type Request struct {
	...
	// RerouteLine: Reroutes a line such that it's connected at the two
	// closest connection sites on the connected page elements.
	...
	// UpdateTableColumnProperties: Updates the properties of a Table
	// column.
...
// RerouteLineRequest: Reroutes a line such that it's connected at the
...
type RerouteLineRequest struct {
	...
	// category indicating it is a "connector" can be rerouted. The start
	// and end connections of the line must be on different page elements.
	ObjectId string `json:"objectId,omitempty"`
...
type Response struct {
	...
	// matching some criteria with a Google Sheets chart.
...
// Shadow: The shadow properties of a page element. If these fields are
// unset, they may be inherited from a parent placeholder if it exists.
// If there is no parent, the fields will default to the value used for
// new page elements created in the Slides editor, which may depend on
// the page element kind.
type Shadow struct {
	// Alignment: The alignment point of the shadow, that sets the origin
	// for translate, scale and skew of the shadow. This property is
	// read-only.
	//
	// Possible values:
	...
	// more diffuse the shadow becomes.
	BlurRadius *Dimension `json:"blurRadius,omitempty"`

	// Color: The shadow color value.
	Color *OpaqueColor `json:"color,omitempty"`

	// PropertyState: The shadow property state. Updating the shadow on a
	// page element will implicitly update this field to `RENDERED`, unless
	// another value is specified in the same request. To have no shadow on
	// a page element, set this field to `NOT_RENDERED`. In this case, any
	// other shadow fields set in the same request will be ignored.
	//
	// Possible values:
	//   "RENDERED" - If a property's state is RENDERED, then the element
	// has the corresponding property when rendered on a page. If the
	...
	// the shadow, relative to the alignment position.
	Transform *AffineTransform `json:"transform,omitempty"`
...
type ShapeBackgroundFill struct {
	// PropertyState: The background fill property state. Updating the fill
	// on a shape will implicitly update this field to `RENDERED`, unless
	// another value is specified in the same request. To have no fill on a
	// shape, set this field to `NOT_RENDERED`. In this case, any other fill
	// fields set in the same request will be ignored.
	//
	// Possible values:
	//   "RENDERED" - If a property's state is RENDERED, then the element
...
// ShapeProperties: The properties of a Shape. If the shape is a
...
type ShapeProperties struct {
	...
	// the alignment for new shapes created in the Slides editor.
	//
	// Possible values:
	//   "CONTENT_ALIGNMENT_UNSPECIFIED" - An unspecified content alignment.
	// The content alignment is inherited from the parent if it exists.
	...
	// link. Links are not inherited from parent placeholders.
	Link *Link `json:"link,omitempty"`

	// Outline: The outline of the shape. If unset, the outline is inherited
	// from a parent placeholder if it exists. If the shape has no parent,
	// then the default outline depends on the shape type, matching the
	...
	// parent, then the default shadow matches the defaults for new shapes
	// created in the Slides editor. This property is read-only.
	Shadow *Shadow `json:"shadow,omitempty"`

	// ShapeBackgroundFill: The background fill of the shape. If unset, the
	// background fill is inherited from a parent placeholder if it exists.
	// If the shape has no parent, then the default background fill depends
	// on the shape type, matching the defaults for new shapes created in
	// the Slides editor.
...
// SheetsChart: A PageElement kind representing a linked chart embedded
// from Google Sheets.
...
type SlideProperties struct {
	...
	// defines the visual appearance of a notes page when printing or
	// exporting slides with speaker notes. A notes page inherits properties
	// from the notes master. The placeholder shape with type BODY on the
	// notes page contains the speaker notes for this slide. The ID of this
	// shape is identified by the speakerNotesObjectId field. The notes page
	// is read-only except for the text content and styles of the speaker
...
// entirely with the specified color value. If any field is unset, its
// value may be inherited from a parent placeholder if it exists.
...
// element is filled entirely with the specified picture. The picture is
// stretched to fit its container.
type StretchedPictureFill struct {
	// ContentUrl: Reading the content_url: An URL to a picture with a
	// default lifetime of 30 minutes. This URL is tagged with the account
	// of the requester. Anyone with the URL effectively accesses the
	// picture as the original requester. Access to the picture may be lost
	// if the presentation's sharing settings change. Writing the
	// content_url: The picture is fetched once at insertion time and a copy
	// is stored for display inside the presentation. Pictures must be less
	// than 50MB in size, cannot exceed 25 megapixels, and must be in one of
	// PNG, JPEG, or GIF format. The provided URL can be at most 2 kB in
	// length.
	ContentUrl string `json:"contentUrl,omitempty"`
...
type Table struct {
	...
	// HorizontalBorderRows: Properties of horizontal cell borders. A
	// table's horizontal cell borders are represented as a grid. The grid
	// has one more row than the number of rows in the table and the same
	// number of columns as the table. For example, if the table is 3 x 3,
	// its horizontal borders will be represented as a grid with 4 rows and
	// 3 columns.
	...
	// TableRows: Properties and contents of each row. Cells that span
	// multiple rows are contained in only one of these rows and have a
	// row_span greater than 1.
	TableRows []*TableRow `json:"tableRows,omitempty"`

	// VerticalBorderRows: Properties of vertical cell borders. A table's
	// vertical cell borders are represented as a grid. The grid has the
	// same number of rows as the table and one more column than the number
	// of columns in the table. For example, if the table is 3 x 3, its
	// vertical borders will be represented as a grid with 3 rows and 4
	// columns.
...
// TableBorderProperties: The border styling properties of the
// TableBorderCell.
...
type TableBorderRow struct {
	...
	// adjacent table cells are merged, it is not included in the response.
...
type TableCellBackgroundFill struct {
	// PropertyState: The background fill property state. Updating the fill
	// on a table cell will implicitly update this field to `RENDERED`,
	// unless another value is specified in the same request. To have no
	// fill on a table cell, set this field to `NOT_RENDERED`. In this case,
...
type TableCellProperties struct {
	...
	// default fill matches the fill for newly created table cells in the
	// Slides editor.
...
// together. The table looks like this: [ ] A table range with location
// = (0, 0), row span = 3 and column span = 2 specifies the following
// cells: x x [ x x x ]
...
type TableRow struct {
	...
	// TableCells: Properties and contents of each cell. Cells that span
	// multiple columns are represented only once with a column_span greater
	// than 1. As a result, the length of this collection does not always
	// match the number of columns of the entire table.
...
type TableRowProperties struct {
	...
	// the Slides editor at a height equal to or greater than this value in
	// order to show all the text in the row's cell(s).
...
// compatible shape (e.g. text box or rectangle) or a table cell in a
// page.
type TextContent struct {
	...
	// including styling information. This property is read-only.
...
// indices in the text content of a Shape or TableCell.
type TextElement struct {
	// AutoText: A TextElement representing a spot in the text that is
	// dynamically replaced with content that can change over time.
	...
	// paragraph. The `start_index` and `end_index` of this TextElement
	// represent the range of the paragraph. Other TextElements with an
	// index range contained inside this paragraph's range are considered to
	// be part of this paragraph. The range of indices of two separate
	...
	// characters in the run have the same TextStyle. The `start_index` and
...
// TextStyle: Represents the styling that can be applied to a TextRun.
// If this text is contained in a shape with a parent placeholder, then
// these text styles may be inherited from the parent. Which text styles
// are inherited depend on the nesting level of lists: * A text run in a
// paragraph that is not in a list will inherit its text style from the
// the newline character in the paragraph at the 0 nesting level of the
// list inside the parent placeholder. * A text run in a paragraph that
// is in a list will inherit its text style from the newline character
// in the paragraph at its corresponding nesting level of the list
// inside the parent placeholder. Inherited text styles are represented
// as unset fields in this message. If text is contained in a shape
// without a parent placeholder, unsetting these fields will revert the
// style to a value matching the defaults in the Slides editor.
type TextStyle struct {
	// BackgroundColor: The background color of the text. If set, the color
	// is either opaque or transparent, depending on if the `opaque_color`
	// field in it is set.
	...
	// font from the Font menu in Slides or from [Google Fonts]
	...
	// either opaque or transparent, depending on if the `opaque_color`
	// field in it is set.
	...
	// to ThemeColorType.HYPERLINK and the text will be underlined. If these
	...
	// point to the new URL. * Links are not settable on newline characters.
	// As a result, setting a link on a text range that crosses a paragraph
	...
	// font weights without breaking backwards compatibility. As such, when
	// reading the style of a range of text, the value of
	...
	// parent. Note that these properties cannot inherit separately from
...
type Thumbnail struct {
	// ContentUrl: The content URL of the thumbnail image. The URL to the
	// image has a default lifetime of 30 minutes. This URL is tagged with
	// the account of the requester. Anyone with the URL effectively
	// accesses the image as the original requester. Access to the image may
	// be lost if the presentation's sharing settings change. The mime type
	// of the thumbnail image is the same as specified in the
...
type UngroupObjectsRequest struct {
	// ObjectIds: The object IDs of the objects to ungroup. Only groups that
	// are not inside other groups can be ungrouped. All the groups should
	// be on the same page. The group itself is deleted. The visual sizes
...
type UnmergeTableCellsRequest struct {
	...
	// the merged cells, the text will remain in the upper-left ("head")
	// cell of the resulting block of unmerged cells.
	TableRange *TableRange `json:"tableRange,omitempty"`
...
type UpdateImagePropertiesRequest struct {
	...
	// field. For example to update the image outline color, set `fields` to
...
type UpdateLineCategoryRequest struct {
	// LineCategory: The line category to update to. The exact line type is
	// determined based on the category to update to and how it's routed to
	...
	// ObjectId: The object ID of the line the update is applied to. Only a
	// line with a category indicating it is a "connector" can be updated.
...
type UpdateLinePropertiesRequest struct {
	...
	// specified. A single "*" can be used as short-hand for listing every
	// field. For example to update the line solid fill color, set `fields`
	// to "lineFill.solidFill.color". To reset a property to its default
...
type UpdatePageElementAltTextRequest struct {
	...
	// unset the existing value will be maintained. The description is
	// exposed to screen readers and other accessibility interfaces. Only
	// use human readable values related to the content of the page element.
	Description string `json:"description,omitempty"`
...
// element. Updating the transform of a group will change the absolute
// transform of the page elements in that group, which can change their
// visual appearance. See the documentation for PageElement.transform
// for more details.
type UpdatePageElementTransformRequest struct {
	...
	// one, and replaces the existing one with the resulting concatenation.
...
// elements. Z-order is an ordering of the elements on the page from
// back to front. The page element in the front may cover the elements
// that are behind it.
type UpdatePageElementsZOrderRequest struct {
	// Operation: The Z-order operation to apply on the page elements. When
	// applying the operation on multiple page elements, the relative
	...
	// one element relative to the forwardmost one in the specified page
	// elements.
	//   "SEND_BACKWARD" - Sends the page elements backward on the page by
	// one element relative to the furthest behind one in the specified page
	// elements.
...
type UpdatePagePropertiesRequest struct {
	...
	// specified. A single "*" can be used as short-hand for listing every
	// field. For example to update the page background solid fill color,
	// set `fields` to "pageBackgroundFill.solidFill.color". To reset a
...
// paragraphs within a Shape or Table that overlap with the given text
// index range.
type UpdateParagraphStyleRequest struct {
	// CellLocation: The location of the cell in the table containing the
	// paragraph(s) to style. If `object_id` refers to a table,
	// `cell_location` must have a value. Otherwise, it must not.
	CellLocation *TableCellLocation `json:"cellLocation,omitempty"`

	// Fields: The fields that should be updated. At least one field must be
	// specified. The root `style` is implied and should not be specified. A
...
type UpdateShapePropertiesRequest struct {
	...
	// specified. A single "*" can be used as short-hand for listing every
	// field. For example to update the shape background solid fill color,
	// set `fields` to "shapeBackgroundFill.solidFill.color". To reset a
	// property to its default value, include its field name in the field
	// mask but leave the field itself unset.
	Fields string `json:"fields,omitempty"`
...
type UpdateSlidesPositionRequest struct {
	...
	// on the slide arrangement before the move takes place. Must be between
	// zero and the number of slides in the presentation, inclusive.
	InsertionIndex int64 `json:"insertionIndex,omitempty"`

	// SlideObjectIds: The IDs of the slides in the presentation that should
	// be moved. The slides in this list must be in existing presentation
...
type UpdateTableBorderPropertiesRequest struct {
	...
	// should apply to. If a border position is not specified, the updates
	...
	// Fields: The fields that should be updated. At least one field must be
	// specified. The root `tableBorderProperties` is implied and should not
	...
	// which the updates are applied. If a table range is not specified, the
	// updates will apply to the entire table.
	TableRange *TableRange `json:"tableRange,omitempty"`
...
type UpdateTableColumnPropertiesRequest struct {
	...
	// columns to update. If no indices are provided, all columns in the
	...
	// "column_width". If '"column_width"' is included in the field mask
	// but the property is left unset, the column width will default to
	...
	// value of `table_column_properties#column_width` in the request is
...
type UpdateTableRowPropertiesRequest struct {
	// Fields: The fields that should be updated. At least one field must be
	// specified. The root `tableRowProperties` is implied and should not be
	// specified. A single "*" can be used as short-hand for listing every
	// field. For example to update the minimum row height, set `fields` to
	// "min_row_height". If '"min_row_height"' is included in the field
	// mask but the property is left unset, the minimum row height will
	// default to 0.
	Fields string `json:"fields,omitempty"`
	...
	// update. If no indices are provided, all rows in the table will be
	// updated.
...
// UpdateTextStyleRequest: Update the styling of text in a Shape or
// Table.
type UpdateTextStyleRequest struct {
	// CellLocation: The location of the cell in the table containing the
	// text to style. If `object_id` refers to a table, `cell_location` must
	// have a value. Otherwise, it must not.
	CellLocation *TableCellLocation `json:"cellLocation,omitempty"`

	// Fields: The fields that should be updated. At least one field must be
	// specified. The root `style` is implied and should not be specified. A
	// single "*" can be used as short-hand for listing every field. For
	// example, to update the text style to bold, set `fields` to "bold".
	...
	// Style: The style(s) to set on the text. If the value for a particular
	// style matches that of the parent, that style will be set to inherit.
	// Certain text style changes may cause other changes meant to mirror
	// the behavior of the Slides editor. See the documentation of TextStyle
	// for more information.
	Style *TextStyle `json:"style,omitempty"`

	// TextRange: The range of text to style. The range may be extended to
...
type UpdateVideoPropertiesRequest struct {
	...
	// specified. A single "*" can be used as short-hand for listing every
	// field. For example to update the video outline color, set `fields` to
	// "outline.outlineFill.solidFill.color". To reset a property to its
	// default value, include its field name in the field mask but leave the
	// field itself unset.
	Fields string `json:"fields,omitempty"`
...
type VideoProperties struct {
	...
	// in present mode. Defaults to false.
	...
	// start time. If not set or if you set this to a value that exceeds the
	// video's length, the video will be played until its end.
	...
	// defaults for new videos created in the Slides editor.
	Outline *Outline `json:"outline,omitempty"`

	// Start: The time at which to start playback, measured in seconds from
	// the beginning of the video. If set, the start time should be before
	// the end time. If you set this to a value that exceeds the video's
	// length in seconds, the video will be played from the last second. If
	// not set, the video will be played from the beginning.
...
// BatchUpdate: Applies one or more updates to the presentation. Each
...
// editing the presentation, the presentation might not exactly reflect
// your changes: your changes may be altered with respect to
...
// request. If a `presentationId` is provided, it is used as the ID of
// the new presentation. Otherwise, a new ID is generated. Other fields
// in the request, including any provided content, are ignored. Returns
...
// thumbnail image. If you don't specify the mime type, the mime type
// defaults to PNG.
//
// Possible values:
...
// size. If you don't specify the size, the server chooses a default
// size of the image.
//
// Possible values:
